...
# --- Blocked coins from API ---
def get_blocked_coins():
    try:
        r = requests.get(f"{API_BASE}/api/blocked-coins", timeout=5)
        r.raise_for_status()
        return {coin['symbol'].upper() for coin in r.json()}
    except Exception as e:
        print("Blocked coins alma hatası:", e)
        return set()
...
# --- Klines & RSI ---
def fetch_klines_binance(symbol: str, interval: str = "1m", limit: int = 200):
    try:
        r = requests.get(
...
# --- Symbol processing ---
def process_symbol(symbol: str, blocked_coins: set):
    # Check if coin is blocked
    if symbol.upper() in blocked_coins:
        return False
    ...
    df1 = fetch_klines_binance(symbol, "1m", limit=KLN_LIMIT)
    if df1 is None or len(df1) < RSI_PERIOD + 1:
        return False

    df1['rsi'] = rsi_wilder(df1['close'], RSI_PERIOD)
    rsi_now = float(df1['rsi'].iloc[-1])
    ...
    previous_price = float(df1['close'].iloc[-2])
    boost = ((current_price - previous_price) / previous_price) * 100 if previous_price != 0 else 0.0

    rsi_5m = rsi_1h = 0.0
    d5 = fetch_klines_binance(symbol, "5m", limit=RSI_PERIOD+10)
    if d5 is not None and len(d5) >= RSI_PERIOD+1:
        d5['rsi'] = rsi_wilder(d5['close'], RSI_PERIOD)
    ...
    message = (
        ...
        f"RSI: 1m.{rsi_now:.2f} | 5m.{rsi_5m:.2f} | 1h.{rsi_1h:.2f}\n"
...
def main_loop():
    while True:
        # Get blocked coins from API
        blocked_coins = get_blocked_coins()
        if blocked_coins:
            print(f"Engellenen coinler: {', '.join(sorted(blocked_coins))}")
        ...
        with ThreadPoolExecutor(max_workers=MAX_WORKERS) as executor:
            futures = {executor.submit(process_symbol, sym, blocked_coins): sym for sym in symbols}
...
if __name__ == "__main__":
    print("Binance Futures RSI Bot (USDT-M) başlatılıyor... (1m)")
    main_loop()
