...
def is_valid_funding_rate(rate):
    try:
        ...
        return (MIN_THRESHOLD <= abs(rate_percent) <= MAX_THRESHOLD)
    except:
        return False
...
async def send_funding_message(symbol, rate):
    ...
    # TradingView link düzeltme — her zaman açilir
    tradingview_symbol = f"BINANCE:{symbol}"
    tradingview_link = f"https://www.tradingview.com/chart/?symbol={quote(tradingview_symbol)}"
